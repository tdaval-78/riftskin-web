alter table public.daily_injection_usage
    drop constraint if exists daily_injection_usage_device_id_window_start_window_end_key;
alter table public.daily_injection_usage
    add column if not exists usage_date date;
update public.daily_injection_usage
set usage_date = ((coalesce(window_start, created_at) at time zone 'Europe/Paris')::date)
...
begin
    if not exists (
        select 1
        from pg_constraint
        where conname = 'daily_injection_usage_device_id_usage_date_key'
    ) then
        alter table public.daily_injection_usage
            add constraint daily_injection_usage_device_id_usage_date_key unique (device_id, usage_date);
    end if;
end $$;
...
declare
    ...
    v_today date := (v_now at time zone 'Europe/Paris')::date;
...
begin
    ...
    if v_access_mode = 'free' then
        ...
        v_window_end := ((v_today + 1)::timestamp at time zone 'Europe/Paris');
